...
export function useDeviceDetect() {
  ...
  useEffect(() => {
    ...
    const mobile = Boolean(
      userAgentData.match(/Android|BlackBerry|iPhone|iPad|iPod|Opera Mini|IEMobile|WPDesktop/i),
    )
    setMobile(mobile)
  }, [])
...
export function createInners(n: number, array: number[]) {
  array.push(n)
  n--
  n ? createInners(n, array) : null
}
